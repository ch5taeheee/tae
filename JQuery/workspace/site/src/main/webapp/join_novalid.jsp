	...
	<script>
		...
		$('#id').on('keyup', function() {
			...
			$.ajax({
				...
				data : {
					'uId' : userId
				}, //객체로 넘겨도 자체적으로 urlencoding후 데이터 전송
		...
		function valid() {
			...
			event.preventDefault();//submit기능 중지 => 지정된 action또는 현재 url로 submit
			...
			$.ajax({
				...
				success : function(result) {
					...
					if (result == 1) {
						...
						//alert("가입성공!!");
						confirm("가입성공했는데, 로그인페이지로 이동할래?");
						if(conf){ //boolean 값을 반환
						location.href = "login.html";
						}
						$('#joinspan').html("가입실패..").css('color', 'gold');
