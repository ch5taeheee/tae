...
public class MultiChatServer {
    // 대화명, 클라이언트의 Socket 을 저장하기 위한 Map 변수 선언
    ...
    /**
     * 대화방 즉 Map 에 저장된 전체 유저에게 안내 메시지를 전송하기 위한 메서드
     *
     * @param msg 전송할 안내 메시지
     */
    public void sendMessage(String msg) {
        // Map 에 저장된 사용자의 대화명 리스트를 추출
        Iterator<String> it = clients.keySet().iterator();

        while (it.hasNext()) {
            try {
                String name = it.next();
                // 대화명에 해당하는 Socket 의 OutputStream 객체 구하기
    ...
    /**
     * 서버에서 클라이언트로 메시지를 전송할 Thread 를 Inner 클래스로 정의한다
     * Inner 클래스에서는 부모(Outer) 클래스의 멤버들을 직접 사용할 수 있다.
     */
