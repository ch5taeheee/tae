...
public class T03ListSort { //파일 하나에 public은 하나
	public static void main(String[] args) {
		/*
		 * 정렬과 관련된 Interface는 Comparable과 Comparator가 있다.
		 * 
		 * - 보통 객체 자체에 정렬 기능을 넣기 위해서는 Comparable을 구현하고 (나랑 파라미터) 정렬 기준을 별도로 구현하고 싶을 때는
		 * Comparator를 구현하여 (파라미터 2개) 사용하면 된다.
		 * 
		 * - Comparable에서는 compareTo()메서드를 구현해야 하고, Comparator 에서는 compare()메서드를 구현해야 한다.
		 * 
		 */

//		객체 생성없이 필요한 기능만을 호출할 때 -> static
		...
		/*
		* 정렬방식을 저장하는 class는 Comparator라는 인터페이스를 구현해야 한다.
...
class Desc implements Comparator<String> {
		...
		* 
		* - String 객체에는 정렬을 위해 compareTo()메서드가 구현되어 있는데 
